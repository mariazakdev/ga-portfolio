...
//       'The texture artifact fully obscures the background layer (Actual Result). Expected Result: The texture artifact should not fully obscure the background layer.',
...
//       'Cape Jake clips through and then falls (Actual Result). Expected Result: Cape Jake should not clip through the floor and then fall.',
//     ],
//     video: bug2Video,
//   },
// {
...
//       'You will notice that after pressing the "Return to the Main Menu" button, it will switch to the incorrect scene, i.e. the Letter Game tutorial scene (Actual Result).Expected Result: The "Return to the Main Menu" button should switch to the correct scene, i.e. the "Main Menu" scene.',
//     ],
//     video: bug3Video,
//   },
        ...
        {BUGS.map((b, index) => (
          <article key={b.id} className="bugs__item">
            <header className="bugs__item-header">
              ...
              <div className="bugs__item-chips">
                <span
                  className={`bugs__item-chip bugs__item-chip--${b.severity.toLowerCase()}`}
                >
                  {b.severity}
                </span>
                <span className="bugs__item-chip bugs__item-chip--neutral">
                  {b.platform}
                </span>
                <span className="bugs__item-chip bugs__item-chip--neutral">
                  {b.status}
            ...
            <div className="bugs__item-layout">
              ...
              <div className="bugs__item-body">
                ...
                <div className="bugs__item-block">
                  <div className="bugs__item-label">Repro Steps</div>
                  <div className="bugs__item-steps">
                    {/* Numbered step paragraphs */}
                    {b.steps.map((s, i) => (
                      <p key={i} className="bugs__item-step">
                        {i + 1}. {s}
                      </p>
                    ))}

                    {/* Unnumbered Expected Result paragraph */}
                    {b.expectedResult && (
                      <p className="bugs__item-expected">
                        {b.expectedResult}
                      </p>
                    )}
                  </div>
